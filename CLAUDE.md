...
Reth is a high-performance Ethereum execution client written in Rust, focusing on modularity, performance, and contributor-friendliness. The codebase is organized into well-defined crates with clear boundaries and responsibilities.
 Reth Codebase    
  Reth is a high-performance Ethereum execution client written in Rust. It's designed around modularity, performance,   
  and extensibility with 180+ crates organized into logical subsystems.                                                 
                                                                                                                        
  Top-Level Structure                                                                                                   
                                                                                                                        
  reth/                                                                                                                 
  ├── bin/           # Main binaries (reth, op-reth, benchmarks)                                                        
  ├── crates/        # Core implementation (~39 directories)                                                            
  ├── examples/      # 36+ usage examples                                                                               
  ├── testing/       # Test utilities                                                                                   
  └── docs/          # Documentation                                                                                    
                                                                                                                        
  Core Architecture (9 Major Components)                                                                                
  ┌──────────────┬───────────────────┬────────────────────────────────────────────┐                                     
  │  Component   │     Location      │                  Purpose                   │                                     
  ├──────────────┼───────────────────┼────────────────────────────────────────────┤                                     
  │ Node Builder │ crates/node/      │ High-level node orchestration              │                                     
  ├──────────────┼───────────────────┼────────────────────────────────────────────┤                                     
  │ Engine       │ crates/engine/    │ Consensus layer communication (Engine API) │                                     
  ├──────────────┼───────────────────┼────────────────────────────────────────────┤                                     
  │ Consensus    │ crates/consensus/ │ Block validation rules                     │                                     
  ├──────────────┼───────────────────┼────────────────────────────────────────────┤                                     
  │ Storage      │ crates/storage/   │ MDBX + static files hybrid database        │                                     
  ├──────────────┼───────────────────┼────────────────────────────────────────────┤                                     
  │ Networking   │ crates/net/       │ P2P, discovery, transaction propagation    │                                     
  ├──────────────┼───────────────────┼────────────────────────────────────────────┤                                     
  │ Pipeline     │ crates/stages/    │ Staged sync architecture                   │                                     
  ├──────────────┼───────────────────┼────────────────────────────────────────────┤                                     
  │ EVM          │ crates/evm/       │ Transaction execution via revm             │                                     
  ├──────────────┼───────────────────┼────────────────────────────────────────────┤                                     
  │ Trie         │ crates/trie/      │ Merkle Patricia Trie, state roots          │                                     
  ├──────────────┼───────────────────┼────────────────────────────────────────────┤                                     
  │ RPC          │ crates/rpc/       │ JSON-RPC server (all Ethereum APIs)        │                                     
  └──────────────┴───────────────────┴────────────────────────────────────────────┘                                     
  Key Subsystems                                                                                                        
                                                                                                                        
  - Transaction Pool (crates/transaction-pool/): Mempool with validation, blob storage                                  
  - Payload Builder (crates/payload/): Block construction for consensus layer                                           
  - ExEx (crates/exex/): Execution extensions (plugins for rollups, indexers)                                           
  - Optimism (crates/optimism/): OP Stack support via op-reth binary                                                    
  ...
  Consensus Layer (Beacon)                                                                                              
      ...
      Engine API (newPayload, forkchoiceUpdated)                                                                        
          ↓                                                                                                             
      Engine Tree → Chain State                                                                                         
          ↓                                                                                                             
      EVM Execution → State Changes                                                                                     
          ↓                                                                                                             
      Trie Root → Persistence                                                                                           
          ↓                                                                                                             
      Storage (MDBX + Static Files)                                                                                     
                                                                                                                        
  Design Principles                                                                                                     
                                                                                                                        
  1. Modularity - Each crate works standalone                                                                           
  2. Performance - Parallelism (rayon), memory-mapped I/O                                                               
  3. Type Safety - Strong typing, minimal dynamic dispatch                                                              
  4. Non-blocking - Critical path avoids DB writes; persistence is background                                           
  5. Extensibility - Traits allow custom implementations (Ethereum, Optimism, custom EVMs)        
  Main Entry Points                                                                                                     
                                                                                                                        
  - bin/reth/ - Main Ethereum client binary                                                                             
  - crates/optimism/bin/ - OP Stack client (op-reth)                                                                    
  - examples/ - Demonstrate customization (custom-node, custom-evm, exex examples)                                      
